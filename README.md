...
`npm test` also runs a standing negative fixture suite (`tools/run-fixture-tests.mjs`). The retained invalid corpora under `tests/fixtures/qif-package/`, `tests/fixtures/expert-judgment/`, `tests/fixtures/discovery-session/`, `tests/fixtures/organizational-quality-culture/`, `tests/fixtures/evaluation-target/`, `tests/fixtures/review-run/`, `tests/fixtures/qif-ledger/`, `tests/fixtures/world-model-review/`, `tests/fixtures/world-model-calibration/`, `tests/fixtures/world-model-pilot-corpus/`, `tests/fixtures/guided-elicitation/`, `tests/fixtures/world-model-elicitation/`, `tests/fixtures/action-quality-contract/`, and `tests/fixtures/quality-gate/` must each be rejected with a specific error, so a covered verifier rule that is silently weakened or deleted fails the build. The committed corpora are generated from `tools/fixtures/*-cases.mjs` by `npm run build-fixtures`; the suite fails on drift if generated source of truth and committed files disagree. v0.6.27 retains 611 negative checks overall, including broken Evidence Origin references and unverified empirical-origin declarations (see `docs/qif-roadmap.md`).
...
node tools/qif.mjs new action-quality-contract --out local-action-contract.json
...
`qif validate` routes each package to the correct local verifier from its `packageType` or legacy package shape. `qif new <package-type>` emits a validated starter package from committed examples, or writes it with `--out` without overwriting an existing file. `qif trace <entity-id>` finds matching entities, outbound references, and inbound references across the provided packages or all committed examples. `qif open-risks` lists unresolved governance triggers, residual-risk carriers, and low-confidence entities. `qif release-ready` runs the local release-ready hook through the canonical CLI and exits 0 only when a quality-gate package validates and contains an accountable Go or Conditional Go decision with required release controls. `qif doctor` runs a one-command structural health check over validation, retained fixtures, release readiness, and open-risk visibility. `qif review-plan` composes status, inventory, release-ready, and open-risk signals into required checks, evidence refs, unresolved-risk prompts, and reviewer-boundary language before release, pilot, or governance work. `qif evaluator-packet` packages command discovery, package types, inventory, status, review-plan, release-ready, open-risk summaries, freshness hints, required commands, evidence refs, and trust boundaries for independent evaluators, AI agents, and audit harnesses. `qif calibration-readiness` checks whether selected quality-gate, ledger, calibration, and pilot-corpus packages contain the declared prerequisites for empirical calibration, resolves each outcome and case to a schema-backed Evidence Origin, reports exact blockers, and treats committed examples as example-only evidence. `qif commands` returns the canonical command surface as machine-readable JSON for humans, AI agents, and harnesses. `qif package-types` returns supported package types, source templates, validators, purposes, lifecycle roles, and verifier boundaries as machine-readable JSON. `qif inventory` returns package ids, package types, entity collection counts, entity counts, outbound reference counts, warnings, cache metadata, and verifier boundaries before an agent selects validation, trace, review, or governance actions. `qif status` returns one machine-readable current-state packet with version, command surface, package type catalog, structural health, inventory summary, open-risk summary, roadmap frontier, trend rationale, and next recommended action. `qif commands`, `qif package-types`, `qif inventory`, `qif review-plan`, `qif evaluator-packet`, `qif calibration-readiness`, and `qif status` include `cache` metadata with freshness, private cache scope, and invalidation hints for stateless agent clients. CLI and hook success prove structural validity, starter-shape generation, reference visibility, risk-carrier extraction, package inventory visibility, review-plan organization, evaluator handoff organization, calibration prerequisite visibility, gate-readiness structure, or local status visibility only; they do not claim semantic quality truth or empirical calibration.
